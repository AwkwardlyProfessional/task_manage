...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("VERCEL_POSTGRES_URL", "sqlite:///default.db")
...
class EventForm(FlaskForm):
    ...
    completed = IntegerField('Completed', default=0)  
    priority = IntegerField('Priority', default = 5) 
...
def send_email(user_email, event):
    ...
    # You can attach the Event object record as a file
...
def send_overdue_emails():
    with app.app_context():
        ...
        print(overdue_events_by_user_id)
        for user_id, event_ids_str in overdue_events_by_user_id:
